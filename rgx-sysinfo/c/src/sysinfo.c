...
#define UPLOAD_TIMER_INTERVAL (30)
...
TSysInfoUploader_Initialize(TSysInfoUploader* self, sse_char* in_urn) {
  ...
  APP_LOG_DEBUG("TSysInfoUploader_Initialize enter");
  ...
  APP_LOG_DEBUG("TSysInfoUploader_Initialize leave");
...
TSysInfoUploader_Finalize(TSysInfoUploader* self) {
  APP_LOG_DEBUG("TSysInfoUploader_Finalize enter");
  moat_object_remove_all(self->fObjects);
  moat_object_free(self->fObjects);
  sse_free(self->fServiceId);
  moat_remove_model(self->fMoat, MOAT_MODEL_NAME);
  moat_destroy(self->fMoat);
  APP_LOG_DEBUG("TSysInfoUploader_Finalize leave");
}
...
TSysInfoUploader_Run(TSysInfoUploader* self) {
  ...
  APP_LOG_DEBUG("TSysInfoUploader_Run enter");
  ...
  APP_LOG_DEBUG("TSysInfoUploader_Run leavee");
...
TSysInfoUploader_Quit(TSysInfoUploader* self) {
  APP_LOG_DEBUG("TSysInfoUploader_Quit enter");
  moat_quit(self->fMoat);
  APP_LOG_DEBUG("TSysInfoUploader_Quit leave");
  return SSE_E_OK;
}
